...
```{r Loading of the data, include=F}
...
```{r Do TF analysis functions, include = F}
...
```{r Boxplot gene expression function, include = F}
...
Make_gene_expr_boxplots <- function(RNAseq, Gene_to_focus, Gene_name, Comparison_A, Comparison_A_name, Comparison_B, Comparison_B_name, Phenotype){
  ...
  # ggsave(paste0(Gene_to_focus, "_", Comparison_A_name, "_", Comparison_B_name, "_boxplots.png"))
...
```{r Making box plot of Expression}
...
```{r Network signatures functions, include = F}
...
```{r Loading combated REACTOME & Dorothea Network, include = F}
...
```{r Generate Cell lines network signature, include = F}
...
write.csv(MOLM14_Mut_IDHi_vs_no_treat_network$features, "~/tmp/MOLM14_Mut_IDHi_vs_no_treat_features.csv", quote = F, sep = "\t")
